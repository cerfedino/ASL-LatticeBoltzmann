...
inline int scalar_index(int y, int l, int x) {
  switch (l) {
  case 0:
    break;
  case 1:
    x = (x - timestep - 1 + Nt * Nx) % Nx;
    break;
  case 2:
    x = (x - timestep - 1 + Nt * Nx) % Nx;
    y = (y - timestep - 1 + Nt * Ny) % Ny;
    break;
  case 3:
    y = (y - timestep - 1 + Nt * Ny) % Ny;
    break;
...
inline int run() {
  ...
  for (int profile_counter = 0; profile_counter < PROFILE_RUNS; profile_counter++) {
    ...
    for (timestep = 0; timestep < Nt; timestep++) {
      do_timestep();
      debug_printf("\r%d", timestep);

#ifdef DEBUG
      char vortex_filename[100];
      sprintf(vortex_filename, "%s/vorticity_%05d.npy", folder_name.c_str(), timestep);
