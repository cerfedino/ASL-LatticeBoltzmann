...
void output_velocity() {
  ...
  for (int x = 0; x < NX; x++) {
    for (int y = 0; y < NY; y++) {
      std::cout << velocity_field_x[scalar_index(x, y, z_index)] << " ";
    }
    std::cout << std::endl;
  }
  std::cout << std::endl;
...
void output_lbm_data(std::string filename, bool header) {
  ...
  for (int x = 0; x < NX; x++) {
    for (int y = 0; y < NY; y++) {
      for (int z = 0; z < NZ; z++) {
        output_stream << density_field[scalar_index(x, y, z)] << "," << velocity_field_x[scalar_index(x, y, z)] << "," << velocity_field_y[scalar_index(x, y, z)] << "," << velocity_field_z[scalar_index(x, y, z)] << '\n';
      }
    }
  }
  output_stream.close();
